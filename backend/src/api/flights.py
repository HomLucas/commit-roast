...
async def search_flights(
    ...
    try:
        ...
        if not all_results and request.include_alternative_sources:
            try:
        ...
        error_fares = deal_detector.detect_error_fares(
            analyzed_results,
            "domestic" if deal_detector.is_domestic(request.origin, request.destination) else "international"
        )
        ...
        return FlightSearchResponse(
            ...
            results=[FlightDealResponse(**r) for r in analyzed_results[:request.limit or 50]],
